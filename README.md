...
延迟采用 Komari 的 TCP 高延迟复测规则；三网共用一张图，点击图例切换显示，图例旁可查看检测失败率。底部时间窗口支持缩放与拖动；“平滑”仅压缩异常峰值的显示范围，选点仍显示原值，失败点保持留空。历史保留 30 天，较长时间范围自动汇总。站点描述用于页面元信息，不在首页正文显示；页脚留空时隐藏。
